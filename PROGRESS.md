...
GitHub Actions (07:30 KST)
        ...
        ▼
  collect_all()
  ├── fetch_youtube_recent()   YouTube RSS → transcript 추출
  └── fetch_rss_recent()       RSS 피드 (48h 필터)
        │
        ▼
  _deduplicate()               Supabase seen_urls bulk 조회
        │
        ▼
  filter_and_analyze()         Gemini / Groq → relevance 점수 필터 (≥7)
        ...
        ▼
  deliver()
  ├── send_telegram_digest()   텍스트 메시지 + 인라인 피드백 버튼
...
**H. 소스 다양화: arXiv / HackerNews**
arXiv cs.LG, stat.ML 카테고리 RSS는 논문 요약 → `one_line_summary`에 "어떤 문제를 어떻게 풀었나" 패턴으로 특화 프롬프트 추가 필요. HackerNews `who's hiring` 스킵 필터도 필요.
