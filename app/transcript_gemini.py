...
- **화제가 바뀌는 지점마다 줄을 바꾸고 그 줄 맨 앞에 `[MM:SS]` 형식으로 영상
  내 시각을 적으세요.** 대략 30초~1분 간격이면 충분하고, 문장마다 붙일 필요는
  없습니다. 1시간이 넘는 영상은 `[H:MM:SS]`로 적으세요.
- "다음은 전사입니다" 같은 머리말이나 맺음말을 붙이지 말고 전사 본문만 출력하세요.

출력 예:
[00:00] 안녕하세요, 오늘은 데이터 생명주기에 대해 발표하겠습니다.
[00:42] 먼저 수집 단계부터 보겠습니다. API 요청이나 시스템 이벤트를 통해...
"""

# 왜 전사에 타임스탬프를 요구하는가:
#   분석 프롬프트는 "자막에 실제로 등장한 [MM:SS]만 쓰고 없으면 null"을 지시한다.
#   그런데 Gemini 전사는 타임스탬프 없는 산문이었다 — 실측 43건 중 42건에 시간
#   표기가 0개였다. 그래서 모델은 지시를 정확히 지켜 **항상 null**을 냈고,
#   YouTube 타임스탬프 보유율이 0/52(0%)로 바닥을 쳤다(evals FAIL 항목).
#
#   즉 품질 저하가 아니라 **능력의 소실**이었다. 구버전 youtube-transcript-api는
#   시간이 찍힌 세그먼트를 주는데, datacenter IP 차단으로 Gemini 전사로 갈아타면서
#   그 정보가 입력에서 사라졌다. 분석 단계를 아무리 조여도 없는 정보는 못 만든다.
#   복구는 입력을 고치는 쪽, 즉 여기서 해야 한다.
